...
for(kkk in 85:NN){ 
  ...
  aggregate_long_keep_names <- function(data, kk = 3, marker_cols = c("Y1","Y2","Y3")) {
    data %>%
      group_by(id) %>%
      arrange(obstime, .by_group = TRUE) %>%
      mutate(block = ceiling(row_number() / kk)) %>%
      group_by(id, block) %>%
      summarise(
        # mean of the block
        obstime = mean(obstime, na.rm = TRUE),
        x1 = last(x1),   # keep last value
        x2 = last(x2),
        w1 = last(w1),
        w2 = last(w2),
        Event = last(Event),
        survtime = last(survtime),
        across(all_of(marker_cols), ~mean(.x, na.rm = TRUE)), # mean of markers
        # last obs in block
        obstime_last = last(obstime),
        across(all_of(marker_cols), ~last(.x), .names = "{.col}_last"),
        .groups = "drop"
      )
  }
  
  # Usage example
  long_agg <- aggregate_long_keep_names(long.data, kk = 5, marker_cols = c("Y1","Y2","Y3"))
  head(long_agg)
...
for(kkk in c(1:NN)[-c(10,47,48,84)]){ 
...
mean(Time1[-c(10,47,48,84)]);sd(Time1[-c(10,47,48,84)])

 
#xtable::xtable(cbind(apply(Cr1[-c(10,47,48,84),],2,mean),apply(Cr1[-c(10,47,48,84),],2,sd)),digits=3)
