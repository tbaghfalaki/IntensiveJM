...
for(kkk in 1:NN){ 
  ...
  aggregate_long <- function(data, kk = 3) {
    data %>%
      group_by(id) %>%
      arrange(obstime, .by_group = TRUE) %>%
      mutate(block = ceiling(row_number() / kk)) %>%   # group every kk rows
      group_by(id, block) %>%
      summarise(
  ...
  # Example usage:
  long.data.mean3 <- aggregate_long(long.data, kk = 3)
  long.data.mean5 <- aggregate_long(long.data, kk = 5)
  
  long.data=long.data.mean5
...
mean(Time1[-c(58,59)]);sd(Time1[-c(58,59)]);
...
\hline
1 & -0.500 & -0.496 & 0.056 & -0.008 & 0.056 & 0.900 \\ 
2 & 0.200 & 0.204 & 0.072 & 0.020 & 0.072 & 0.980 \\ 
3 & -0.200 & -0.242 & 0.141 & 0.210 & 0.146 & 0.930 \\ 
\hline
\end{tabular}
\end{table}
> 
