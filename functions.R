...
#
# NOTE: this function depends on funData() and PACE(), which come from the
# `funData` and `MFPCA` packages respectively. Neither package is loaded by
# default here (the main simulation script uses refund::fpca.sc instead and
# never calls uPACE/mFPCA), so if you do call this function, load the
# packages first:
#   library(funData)
#   library(MFPCA)
...
plot.tdROC <- function( fm ) {
  ...
  #  -- a plot of ROC curve
  #
  # FIX: windows() only exists on Windows and errors on macOS/Linux.
  # dev.new() opens a new plotting device on any platform.
  x <- 1 - fm$ROC$spec ;
  y <- fm$ROC$sens ;
  tmp <- order(x, y) ;
  x2 <- x[tmp] ;
